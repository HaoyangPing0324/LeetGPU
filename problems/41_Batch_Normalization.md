...
[Original LeetGPU Challenge](https://leetgpu.com/challenges/batch-normalization)
...
For each feature channel j, batch normalization computes: $$ \begin{align} \mu_j &= \frac{1}{N} \sum\_{i=1}^{N} x\_{i,j} \\ \sigma_j^2 &= \frac{1}{N} \sum\_{i=1}^{N} (x\_{i,j} - \mu_j)^2 \\ \hat{x}\_{i,j} &= \frac{x\_{i,j} - \mu_j}{\sqrt{\sigma_j^2 + \epsilon}} \\ y\_{i,j} &= \gamma_j \hat{x}\_{i,j} + \beta_j \end{align} $$
